...
Nelle pipeline applicative reali, la presenza di imperfezioni nei dati — come
...
quantificare la resilienza di ciascun algoritmo al progressivo degrado delle 
...
Il fine ultimo non è decretare un singolo modello vincitore, ma comprendere le 
dinamiche e le cause del degrado prestazionale in funzione del tipo di 
disturbo. Il protocollo sperimentale prevede la definizione di una baseline sui 
dati integri; successivamente, i modelli vengono riaddestrati su versioni del 
training set alterate a tre livelli di intensità crescente: 10\%, 25\% e 40\%. 
Le metriche di valutazione vengono sempre calcolate su un test set mantenuto 
pulito e inalterato. I tre modelli analizzati sono una rete neurale standard 
(\textit{NN Base}), una variante dotata di meccanismi di regolarizzazione 
(\textit{NN Pro}) e un \textit{Random Forest}. I sette disturbi introdotti 
agiscono su feature, etichette, valori mancanti, outlier e disponibilità 
stessa delle variabili. Viene inoltre integrato un esperimento non 
supervisionato (K-Means) per osservare l'impatto del rumore quando il modello 
non ha accesso alle etichette.
...
(raggi cosmici). Un adrone classificato erroneamente come gamma (falso positivo) 
contamina il segnale, mentre un gamma scartato (falso negativo) riduce la 
statistica utile per le analisi fisiche. Questa forte asimmetria nei costi 
degli errori rende la semplice accuratezza una metrica insufficiente per 
valutare adeguatamente un modello.
...
la distribuzione di intensità dell'immagine dell'evento sul rivelatore. In fase 
...
per i gamma (segnale). La Tabella~\ref{tab:feature} riporta la descrizione 
